...
namespace DMEngine{
short& ReverseByteOrder(short& val){
	return val =	(val & 0x00FFu) << 010 |
					(val & 0xFF00u) >> 010 ;
}
int& ReverseByteOrder(int& val){
	return val =	(val & 0x000000FFu) << 030 |
					(val & 0x0000FF00u) << 010 |
					(val & 0x00FF0000u) >> 010 |
					(val & 0xFF000000u) >> 030 ;
}
long& ReverseByteOrder(long& val){
	return val = 	(val & 0x000000FFul) << 030 |
					(val & 0x0000FF00ul) << 010 |
					(val & 0x00FF0000ul) >> 010 |
					(val & 0xFF000000ul) >> 030 ;
}
long long& ReverseByteOrder(long long& val){
	return val =	(val & 0x00000000000000FFull) << 070 |
...
unsigned long& ReverseByteOrder(unsigned long& val){
	return val = 	(val & 0x000000FFul) << 030 |
					(val & 0x0000FF00ul) << 010 |
					(val & 0x00FF0000ul) >> 010 |
					(val & 0xFF000000ul) >> 030 ;
}
unsigned long long& ReverseByteOrder(unsigned long long& val){
	return val =	(val & 0x00000000000000FFull) << 070 |
					(val & 0x000000000000FF00ull) << 050 |
					(val & 0x0000000000FF0000ull) << 030 |
					(val & 0x00000000FF000000ull) << 010 |
